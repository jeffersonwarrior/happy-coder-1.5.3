...
import { NativeModules, Platform, Pressable, ScrollView, Text, View } from 'react-native';
    ...
    <>
      ...
      <ItemList>
        ...
        <ItemGroup title="Raw Native Module Manifests">
          ...
          />
          {parsedExponentManifest && (
            <JsonViewer title="ExponentConstants.manifest (Embedded)" data={parsedExponentManifest} />
          )}
          {parsedUpdatesManifest && <JsonViewer title="ExpoUpdates.manifest (OTA)" data={parsedUpdatesManifest} />}
          {parsedDevLauncherManifest && <JsonViewer title="DevLauncher.manifest" data={parsedDevLauncherManifest} />}
        </ItemGroup>

        {/* Raw String Manifests (for debugging) */}
        <ItemGroup title="Raw Manifest Strings">
          {typeof rawExponentManifest === 'string' && (
            <JsonViewer title="ExponentConstants.manifest (raw string)" data={{ raw: rawExponentManifest }} />
          )}
          {typeof rawUpdatesManifest === 'string' && (
            <JsonViewer title="ExpoUpdates.manifestString (raw)" data={{ raw: rawUpdatesManifest }} />
          )}
          {rawDevLauncherManifest && (
            <JsonViewer title="DevLauncher.manifestString (raw)" data={{ raw: rawDevLauncherManifest }} />
          )}
        </ItemGroup>

        {/* Resolved App Config */}
        <ItemGroup title="Resolved App Config">
          <JsonViewer title="Loaded App Config (from @/config)" data={config} defaultExpanded={true} />
        </ItemGroup>

        {/* System Constants */}
        <ItemGroup title="System Constants">
          <Item title="Device ID" detail={Constants.deviceId || 'Not available'} showChevron={false} />
          ...
          <Item title="Debug Mode" detail={Constants.debugMode ? 'Yes' : 'No'} showChevron={false} />
          <Item title="App Ownership" detail={Constants.appOwnership || 'N/A'} showChevron={false} />
          <Item title="Execution Environment" detail={Constants.executionEnvironment || 'N/A'} showChevron={false} />
        </ItemGroup>

        {/* Updates Information */}
        <ItemGroup title="Updates Information">
          <Item title="Update ID" detail={updateId || 'Not available'} showChevron={false} />
          <Item title="Release Channel" detail={releaseChannel || 'Not available'} showChevron={false} />
        ...
        <ItemGroup title="Native Modules">
          <Item
            ...
            showChevron={false}
          />
          <Item title="ExpoUpdates" detail={ExpoUpdates ? 'Available' : 'Not available'} showChevron={false} />
          <Item
            title="EXDevLauncher"
            detail={NativeModules.EXDevLauncher ? 'Available' : 'Not available'}
            showChevron={false}
          />
          {ExponentConstants && <JsonViewer title="ExponentConstants (full module)" data={ExponentConstants} />}
