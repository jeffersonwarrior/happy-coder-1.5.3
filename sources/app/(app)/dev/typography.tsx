...
const CodeSample = ({ title, style }: { title: string; style: any }) => (
  <View style={styles.sampleContainer}>
    <Text style={styles.sampleTitle}>{title}</Text>
    <Text style={[{ fontSize: 14 }, style]}>{`const greeting = "Hello, World!";\nconsole.log(greeting);`}</Text>
  </View>
);
    ...
    <ScrollView style={styles.container}>
      <View style={styles.content}>
        ...
        <View style={styles.section}>
          ...
          <TextSample title="Bold (700) - Logo Only" style={{ fontSize: 28, ...Typography.logo() }} text="Happy" />
          <Text style={styles.note}>Note: This font should only be used for the app logo and branding</Text>
        </View>
        ...
        <View style={styles.section}>
          ...
          {[12, 14, 16, 18, 20, 24, 28, 32, 36].map(size => (
            <View key={size} style={styles.fontSizeItem}>
              <Text style={{ fontSize: size, ...Typography.default() }}>{size}px - The quick brown fox</Text>
            </View>
          ))}
        </View>
