...
export default function PathPickerScreen() {
  ...
  const recentPaths = useMemo(() => {
    ...
    if (sessions) {
      ...
      pathsWithTimestamps.sort((a, b) => b.timestamp - a.timestamp).forEach(item => paths.push(item.path));
    }
            ...
            {recentPaths.length === 0 && (
              <ItemGroup title="Suggested Paths">
                {(() => {
                  ...
                  const suggestedPaths = [homeDir, `${homeDir}/projects`, `${homeDir}/Documents`, `${homeDir}/Desktop`];
