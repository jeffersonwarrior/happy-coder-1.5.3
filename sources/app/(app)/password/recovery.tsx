...
export default function PasswordRecoveryScreen() {
  ...
  const [biometricTypes, setBiometricTypes] = React.useState<LocalAuthentication.AuthenticationType[]>([]);
  ...
  const handleBiometricReset = async () => {
    if (!biometricAvailable) {
      Modal.alert(t('password.biometricUnavailable'), t('password.biometricUnavailableMessage'));
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        const confirmed = await Modal.confirm(t('password.resetPasswordTitle'), t('password.resetPasswordBiometric'), {
  ...
  const handleAppReset = async () => {
    const confirmed = await Modal.confirm(t('password.resetAppTitle'), t('password.resetAppWarning'), {
    ...
    const doubleConfirmed = await Modal.confirm(t('password.confirmReset'), t('password.confirmResetMessage'), {
  ...
  return (
    <ScrollView style={[styles.container, { paddingTop: safeArea.top }]} contentContainerStyle={styles.content}>
      ...
      <View style={styles.header}>
        <Ionicons name="help-circle-outline" size={64} color={theme.colors.text} />
        <Text style={[styles.title, { color: theme.colors.text }]}>{t('password.recoveryTitle')}</Text>
      ...
      <View style={styles.optionsContainer}>
        ...
        {biometricAvailable && (
          <Pressable
            style={[styles.optionCard, { backgroundColor: theme.colors.input.background }]}
            onPress={handleBiometricReset}
            disabled={isLoading}
          >
            <View style={styles.optionHeader}>
              <Ionicons name={getBiometricIconName()} size={32} color="#34C759" />
              <View style={styles.optionTextContainer}>
                <Text style={[styles.optionTitle, { color: theme.colors.text }]}>{getBiometricLabel()}</Text>
        ...
        >
          <View style={styles.optionHeader}>
            ...
            <View style={styles.optionTextContainer}>
              <Text style={[styles.optionTitle, { color: theme.colors.text }]}>{t('password.useSecretKey')}</Text>
              <Text style={[styles.optionDescription, { color: theme.colors.textSecondary }]}>
        ...
        <Pressable
          style={[styles.optionCard, styles.destructiveCard, { backgroundColor: theme.colors.input.background }]}
        ...
        >
          <View style={styles.optionHeader}>
            ...
            <View style={styles.optionTextContainer}>
              <Text style={[styles.optionTitle, styles.destructiveText]}>{t('password.resetApp')}</Text>
              <Text style={[styles.optionDescription, { color: theme.colors.textSecondary }]}>
      ...
      <View style={styles.footer}>
        <Text style={[styles.footerText, { color: theme.colors.textSecondary }]}>{t('password.recoveryFooter')}</Text>

        <Pressable style={styles.backButton} onPress={() => router.back()} disabled={isLoading}>
          <Text style={[styles.backButtonText, { color: theme.colors.text }]}>{t('password.backToUnlock')}</Text>
        </Pressable>
      </View>
      ...
      {isLoading && (
        <View style={styles.loadingOverlay}>
          <View style={[styles.loadingContainer, { backgroundColor: theme.colors.surface }]}>
            <Text style={[styles.loadingText, { color: theme.colors.text }]}>{t('password.processing')}</Text>
          </View>
        </View>
      )}
    </ScrollView>
