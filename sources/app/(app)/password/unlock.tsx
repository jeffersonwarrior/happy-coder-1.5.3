  ...
  const handlePasswordSubmit = async () => {
    ...
    try {
      ...
      if (isValid) {
        ...
        if (newAttempts >= 5) {
          Modal.alert(t('password.tooManyAttempts'), t('password.tooManyAttemptsMessage'), [
            {
  ...
  return (
    <View style={[styles.container, { paddingTop: safeArea.top }]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={[styles.title, { color: theme.colors.text }]}>{t('password.unlockSession')}</Text>
      ...
      <View style={styles.form}>
        ...
        {/* Attempt counter */}
        {attempts > 0 && (
          <Text style={[styles.attemptText, { color: theme.colors.textDestructive }]}>
        ...
        <Pressable
          style={[
            styles.unlockButton,
            {
              backgroundColor: password.trim() && !isLoading ? theme.colors.text : theme.colors.input.background,
            },
          ]}
        ...
        >
          {isLoading ? (
            <Text style={[styles.buttonText, { color: theme.colors.textSecondary }]}>{t('password.verifying')}</Text>
          ) : (
            <>
              <Text
                style={[
                  styles.buttonText,
                  {
                    color: password.trim() ? theme.colors.surface : theme.colors.textSecondary,
                  },
                ]}
              >
                {t('password.unlock')}
        ...
        {biometricAvailable && (
          <Pressable
            style={[styles.biometricButton, { borderColor: theme.colors.divider }]}
            onPress={handleBiometricAuth}
            disabled={isLoading}
          >
            <Ionicons
            ...
            />
            <Text style={[styles.biometricText, { color: theme.colors.text }]}>{t('password.useBiometric')}</Text>
          </Pressable>
        )}

        {/* Forgot password */}
        <Pressable style={styles.forgotButton} onPress={handleForgotPassword} disabled={isLoading}>
          <Text style={[styles.forgotText, { color: theme.colors.textSecondary }]}>{t('password.forgotPassword')}</Text>
        </Pressable>
      </View>
    </View>
  );
