...
function FilePreviewModal({ visible, file, sessionId, onClose }: FilePreviewProps) {
  ...
  const isTextFile = (fileName: string): boolean => {
    const extension = fileName.split('.').pop()?.toLowerCase();
    return extension ? SUPPORTED_FILE_EXTENSIONS.has(extension) || TEXT_FILE_EXTENSIONS.has(extension) : false;
  };
  ...
  return (
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <View style={[styles.previewContainer, { backgroundColor: theme.colors.surface }]}>
        {/* Header */}
        <View style={[styles.previewHeader, { borderBottomColor: theme.colors.divider }]}>
          <View style={styles.previewHeaderContent}>
            <View style={styles.previewTitleContainer}>
              <FileIcon fileName={file.fileName} size={24} />
              <View style={styles.previewTitleText}>
                <Text style={[styles.previewTitle, { color: theme.colors.text }]}>{file.fileName}</Text>
        ...
        <ScrollView style={styles.previewContent}>
          {isLoading ? (
            <View style={styles.previewLoadingContainer}>
              ...
              <Text style={[styles.previewLoadingText, { color: theme.colors.textSecondary }]}>Loading file...</Text>
            </View>
          ) : error ? (
            <View style={styles.previewErrorContainer}>
              <Octicons name="alert" size={48} color={theme.colors.textSecondary} />
              <Text style={[styles.previewErrorText, { color: theme.colors.textSecondary }]}>{error}</Text>
            </View>
          ) : !isTextFile(file.fileName) ? (
            <View style={styles.previewErrorContainer}>
              <Octicons name="file-binary" size={48} color={theme.colors.textSecondary} />
              <Text style={[styles.previewErrorText, { color: theme.colors.textSecondary }]}>
                Binary file - preview not available
              </Text>
              <Text style={[styles.previewErrorSubtext, { color: theme.colors.textSecondary }]}>
                Tap the back button and select "Open in file viewer" for full functionality
              </Text>
            </View>
          ) : fileContent ? (
            <View style={styles.previewCodeContainer}>
              {shouldShowSyntaxHighlighting(file.fileName) ? (
                ...
                <Text style={[styles.previewPlainText, { color: theme.colors.text }]} selectable={true}>
                  {fileContent}
                </Text>
              )}
            </View>
            ...
            <View style={styles.previewErrorContainer}>
              <Octicons name="file" size={48} color={theme.colors.textSecondary} />
              <Text style={[styles.previewErrorText, { color: theme.colors.textSecondary }]}>Empty file</Text>
            </View>
          )}
        </ScrollView>
      </View>
...
export default function RepositoryScreen() {
  ...
  const [contextMenuFile, setContextMenuFile] = React.useState<FileItem | null>(null);
  ...
  React.useEffect(() => {
    const loadRecentFiles = async () => {
      try {
        const stored = await AsyncStorage.getItem(`${RECENT_FILES_STORAGE_KEY}_${sessionId}`);
        if (stored) {
  ...
  const addToRecentFiles = React.useCallback(
    (file: FileItem) => {
      setRecentFiles(prev => {
        ...
        try {
          AsyncStorage.setItem(`${RECENT_FILES_STORAGE_KEY}_${sessionId}`, JSON.stringify(updated));
        } catch (error) {
  ...
  const getCurrentFolderItems = React.useCallback((files: FileItem[], path: string, searchTerm: string): FileItem[] => {
    ...
    if (path === '') {
      ...
      return files.filter(file => {
        const pathParts = file.fullPath.split('/').filter(part => part.length > 0);
        return pathParts.length === 1 || (pathParts.length === 2 && file.fileType === 'folder');
      });
    } else {
      ...
      return files.filter(file => {
        ...
        if (file.fullPath.startsWith(pathPrefix)) {
          ...
          return (
            ...
            (relativeParts.length === 2 && file.fileType === 'folder' && relativePath.endsWith('/'))
          );
        }
        return false;
      });
  ...
  React.useEffect(() => {
    if (allFiles.length > 0) {
      const items = getCurrentFolderItems(allFiles, currentPath, debouncedSearchQuery);
  ...
  }, [filteredItems, selectedIndex, showPreview, currentPath, searchQuery, handleItemPress]);
    ...
    <View style={styles.breadcrumbContainer}>
      {breadcrumbs.map((crumb, index) => (
        <React.Fragment key={crumb.path}>
          <Text
            style={[styles.breadcrumb, index === breadcrumbs.length - 1 && styles.breadcrumbActive]}
            onPress={() => handleBreadcrumbPress(crumb.path)}
          >
            {crumb.name}
          </Text>
          {index < breadcrumbs.length - 1 && <Text style={styles.breadcrumbSeparator}> / </Text>}
        </React.Fragment>
      ))}
    </View>
  );
  ...
  const renderFileItem = React.useCallback(
    ({ item, index }: { item: FileItem; index: number }) => (
      <Item
        ...
        onLongPress={item.fileType === 'file' ? handleItemLongPress(item) : undefined}
  ...
  const keyExtractor = React.useCallback((item: FileItem, index: number) => `${item.fullPath}-${index}`, []);

  return (
    <View style={[styles.container, { backgroundColor: theme.colors.surface }]}>
      {/* Search Input */}
      <View style={styles.searchContainer}>
        <View style={[styles.searchInputContainer, { backgroundColor: theme.colors.input.background }]}>
      ...
      {!searchQuery && currentPath === '' && recentFiles.length > 0 && (
        <View style={styles.recentFilesSection}>
          <View style={[styles.recentFilesHeader, { borderBottomColor: theme.colors.divider }]}>
            <Octicons name="clock" size={16} color={theme.colors.textSecondary} />
            <Text style={[styles.recentFilesTitle, { color: theme.colors.text }]}>Recent Files</Text>
          </View>
          {recentFiles.slice(0, 5).map((file, index) => (
            <Item
              ...
              icon={<Octicons name="history" size={24} color={theme.colors.textSecondary} />}
              onPress={() => handleItemPress(file)}
              onLongPress={file.fileType === 'file' ? handleItemLongPress(file) : undefined}
      ...
      <ItemList style={{ flex: 1 }}>
        {isLoading ? (
          <View style={styles.loadingContainer}>
            <ActivityIndicator size="small" color={theme.colors.textSecondary} />
            <Text style={[styles.loadingText, { color: theme.colors.textSecondary }]}>Loading repository...</Text>
          </View>
          ...
          <>
            {searchQuery && (
              <View style={[styles.searchResultsHeader, { backgroundColor: theme.colors.surfaceHigh }]}>
                <Text style={[styles.searchResultsText, { color: theme.colors.textLink }]}>
      ...
      {/* File Preview Modal */}
      <FilePreviewModal visible={showPreview} file={previewFile} sessionId={sessionId} onClose={handleClosePreview} />
