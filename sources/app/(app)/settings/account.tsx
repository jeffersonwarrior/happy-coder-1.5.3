...
import { clearPasswordData, isPasswordProtectionEnabled, setPasswordProtection } from '@/auth/passwordSecurity';
...
export default React.memo(() => {
  ...
  const [disconnecting, handleDisconnectGitHub] = useHappyAction(async () => {
    const confirmed = await Modal.confirm(t('modals.disconnectGithub'), t('modals.disconnectGithubConfirm'), {
  ...
  const handleDisablePassword = async () => {
    const confirmed = await Modal.confirm(t('password.disablePassword'), t('password.disablePasswordMessage'), {
      confirmText: t('password.disablePassword'),
      destructive: true,
    });
      ...
      <ItemList>
        ...
        <ItemGroup title={t('settingsAccount.accountInformation')}>
          <Item
            ...
            detail={
              auth.isAuthenticated ? t('settingsAccount.statusActive') : t('settingsAccount.statusNotAuthenticated')
            }
            showChevron={false}
          />
          <Item
        ...
        {(displayName || githubUsername || profile.avatar) && (
          <ItemGroup title={t('settingsAccount.profile')}>
            {displayName && <Item title={t('settingsAccount.name')} detail={displayName} showChevron={false} />}
            {githubUsername && (
              <Item
        ...
        {profile.connectedServices &&
          ...
          (() => {
            ...
            const displayServices = profile.connectedServices.filter(service => service in knownServices);
        ...
        {/* Analytics Section */}
        <ItemGroup title={t('settingsAccount.privacy')} footer={t('settingsAccount.privacyDescription')}>
          <Item
