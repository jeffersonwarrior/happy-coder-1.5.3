...
import { useAllMachines, useEntitlement, useLocalSettingMutable, useProfile } from '@/sync/storage';
...
function ManualAuthModal({ onClose, onSubmit }: { onClose: () => void; onSubmit: (url: string) => void }) {
  ...
  return (
    ...
    >
      <Text style={{ fontSize: 18, fontWeight: '600', marginBottom: 8 }}>{t('modals.authenticateTerminal')}</Text>
      <Text
        style={{
          fontSize: 14,
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'flex-end' }}>
        <Pressable onPress={onClose} style={{ paddingVertical: 8, paddingHorizontal: 16, marginRight: 8 }}>
        ...
        >
          <Text style={{ color: '#007AFF', fontSize: 16, fontWeight: '600' }}>{t('common.authenticate')}</Text>
        </Pressable>
      </View>
    </View>
  );
  ...
  return (
    <ItemList style={{ paddingTop: 0 }}>
      ...
      <ItemGroup title={t('settings.connectedAccounts')}>
        <Item
          title="Claude Code"
          subtitle={isAnthropicConnected ? t('settingsAccount.statusActive') : t('settings.connectAccount')}
          icon={
            <Image
