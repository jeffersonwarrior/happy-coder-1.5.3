 ...
 */
export async function generateSalt(length: number = DEFAULT_PASSWORD_CONFIG.saltLength): Promise<Uint8Array> {
 ...
 */
export async function decryptWithPassword(encryptedData: string, password: string, saltHex: string): Promise<string> {
  try {
...
export async function storePasswordHash(
  ...
  try {
    ...
    if (Platform.OS === 'web') {
      ...
      await SecureStore.setItemAsync(PASSWORD_STORAGE_KEYS.PASSWORD_HASH, hashHex);
      await SecureStore.setItemAsync(PASSWORD_STORAGE_KEYS.PASSWORD_SALT, saltHex);
    }
...
export async function verifyPassword(password: string): Promise<boolean> {
  try {
    ...
    if (Platform.OS === 'web') {
      ...
      storedHash = await SecureStore.getItemAsync(PASSWORD_STORAGE_KEYS.PASSWORD_HASH);
      storedSalt = await SecureStore.getItemAsync(PASSWORD_STORAGE_KEYS.PASSWORD_SALT);
    }
...
export async function isPasswordProtectionEnabled(): Promise<boolean> {
  try {
    if (Platform.OS === 'web') {
      return localStorage.getItem(PASSWORD_STORAGE_KEYS.PASSWORD_PROTECTED) === 'true';
    } else {
      const result = await SecureStore.getItemAsync(PASSWORD_STORAGE_KEYS.PASSWORD_PROTECTED);
      return result === 'true';
    }
  } catch (error) {
...
export async function setPasswordProtection(enabled: boolean): Promise<void> {
  try {
    if (Platform.OS === 'web') {
      localStorage.setItem(PASSWORD_STORAGE_KEYS.PASSWORD_PROTECTED, enabled.toString());
    } else {
      await SecureStore.setItemAsync(PASSWORD_STORAGE_KEYS.PASSWORD_PROTECTED, enabled.toString());
    }
  ...
  if (Platform.OS === 'ios' || Platform.OS === 'macos') {
    ...
    switch (securityLevel) {
      case SecurityLevel.BIOMETRIC:
        ...
        options.authenticationPrompt = keychainOptions.promptMessage || 'Authenticate to access your secure data';
        break;

      case SecurityLevel.DEVICE_PASSCODE:
        options.requireAuthentication = true;
        options.authenticationPrompt = keychainOptions.promptMessage || 'Enter device passcode';
        break;

      case SecurityLevel.MAXIMUM:
        options.requireAuthentication = true;
        options.authenticationPrompt = keychainOptions.promptMessage || 'Authenticate with biometrics or passcode';
        break;
    ...
    switch (securityLevel) {
      ...
      case SecurityLevel.MAXIMUM:
        options.requireAuthentication = true;
        options.authenticationPrompt = keychainOptions.promptMessage || 'Authenticate to access Happy Coder';
        break;
  ...
  try {
    if (Platform.OS === 'web') {
      ...
      if (securityLevel !== SecurityLevel.BASIC) {
        console.warn('Enhanced security levels not fully supported on web platform');
      }
...
export async function secureStoreGet(
  ...
  try {
    ...
    if (error && typeof error === 'object' && 'code' in error) {
      ...
      if (errorCode === 'UserCancel' || errorCode === 'BiometryNotEnrolled' || errorCode === 'AuthenticationFailed') {
 ...
 */
export async function getAvailableBiometricTypes(): Promise<LocalAuthentication.AuthenticationType[]> {
  try {
    if (Platform.OS === 'web') {
      return [];
    }
