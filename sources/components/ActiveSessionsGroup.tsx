  ...
  return (
    <View style={styles.container}>
      {sortedProjectGroups.map(([projectPath, projectGroup]) => {
        ...
        const machineName =
          projectGroup.machines.size === 1 ? firstMachine?.machineName : `${projectGroup.machines.size} machines`;
  ...
  ({ session, selected, showBorder }: { session: Session; selected?: boolean; showBorder?: boolean }) => {
    ...
    const handleDeleteSession = React.useCallback(async () => {
      const confirmed = await Modal.confirm(
        ...
        t('sessions.deleteSessionMessage', {
    ...
    const handleRenameSession = React.useCallback(async () => {
      const newName = await Modal.prompt(t('sessions.renameSessionTitle'), t('sessions.renameSessionMessage'), {
