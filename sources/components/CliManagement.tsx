...
export function CliManagement({ session }: CliManagementProps) {
  ...
  const isOnDevBranch =
    session.metadata?.version?.includes('git') ||
  ...
  const handleStableUpdate = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        Modal.alert(t('common.success'), t('sessionInfo.updateCliSuccess'), [
          {
            text: t('common.ok'),
            onPress: () => {
      ...
      Modal.alert(t('common.error'), `${t('sessionInfo.updateCliError')}\n\nError: ${String(error)}`);
    } finally {
      setIsUpdating(false);
    }
  }, [session, router]);
  ...
  const handleDevBranchSwitch = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        Modal.alert(t('common.success'), t('sessionInfo.updateCliDevSuccess'), [
          {
            text: t('common.ok'),
            onPress: () => {
              router.replace(`/session/${session.id}`);
  ...
  const handleStableBranchSwitch = useCallback(async () => {
    ...
    try {
      ...
      const commands = ['npm uninstall -g happy-coder', 'npm install -g happy-coder@latest'];
    ...
    <ItemGroup>
      ...
      {isCliOutdated && !isOnDevBranch && (
        <Item
          ...
          subtitle={
            isUpdating ? 'Please wait...' : `Update from v${session.metadata.version} to v${MINIMUM_CLI_VERSION}+`
          }
          icon={
            <Ionicons
