      ...
      <View style={styles.content}>
        {command.icon && (
          <View style={styles.iconContainer}>
            <Ionicons name={command.icon as any} size={20} color={isSelected ? '#007AFF' : '#666'} />
          </View>
        )}
        <View style={styles.textContainer}>
          <Text style={[styles.title, Typography.default()]}>{command.title}</Text>
          {command.subtitle && <Text style={[styles.subtitle, Typography.default()]}>{command.subtitle}</Text>}
        </View>
        {command.shortcut && (
