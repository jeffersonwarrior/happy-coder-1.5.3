...
export function CompactGitStatus({ sessionId }: CompactGitStatusProps) {
  ...
  return (
    <View style={styles.container}>
      <Ionicons name="git-branch-outline" size={10} color={styles.fileCountText.color} style={{ marginRight: 2 }} />

      {/* Show line changes in compact format */}
      {hasLineChanges && (
        <View style={styles.lineChanges}>
          {gitStatus.unstagedLinesAdded > 0 && <Text style={styles.addedText}>+{gitStatus.unstagedLinesAdded}</Text>}
...
function hasMeaningfulChanges(status: GitStatus): boolean {
  ...
  return (
    status.lastUpdatedAt > 0 && status.isDirty && (status.unstagedLinesAdded > 0 || status.unstagedLinesRemoved > 0)
  );
}
