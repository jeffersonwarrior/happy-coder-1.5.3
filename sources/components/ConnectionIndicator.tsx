...
import { type ConnectionHealthStatus, connectionHealthMonitor } from '@/sync/connectionHealth';
...
export const ConnectionIndicator: React.FC<ConnectionIndicatorProps> = ({
  ...
  const latencyDisplay = showLatency && status.latency !== null ? `${status.latency}ms` : '';

  return (
    <View style={[styles.container, compact && styles.compactContainer, style]}>
      <Ionicons name={getStatusIcon()} size={compact ? 12 : 14} color={getStatusColor()} style={styles.icon} />

      {!compact && (
        <>
          <View style={[styles.statusDot, { backgroundColor: getStatusColor() }]} />
          <Text style={styles.statusText}>{getStatusText()}</Text>
          {latencyDisplay && <Text style={styles.latencyText}>{latencyDisplay}</Text>}
        </>
      )}
    </View>
  );
