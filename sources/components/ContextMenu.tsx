...
export const ContextMenu: React.FC<ContextMenuProps> = ({
  ...
  React.useEffect(() => {
    ...
    const estimatedWidth = Math.max(baseWidth, title ? title.length * 8 + 40 : baseWidth);
    ...
    setMenuPosition({ x, y });
  }, [visible, anchorPosition, allActions.length, title, safeArea, normalizedSections]);
    ...
    >
      <View style={styles.actionContent}>
        ...
        {action.shortcut && Platform.OS === 'web' && (
          <Text style={[styles.shortcutText, { color: theme.colors.textSecondary }]}>{action.shortcut}</Text>
        )}
      </View>
    </Pressable>
  );
    ...
    <View key={section.id}>
      ...
      <View style={styles.sectionContent}>
        {section.actions.map((action, actionIndex) =>
          renderAction(
            action,
            actionIndex === section.actions.length - 1 && sectionIndex === normalizedSections.length - 1
          )
        )}
      </View>
  ...
  return (
    ...
    >
      {/* Overlay */}
      <Pressable style={styles.overlay} onPress={handleOverlayPress} accessible={false}>
        {/* Menu Container */}
        <Animated.View
          ref={menuRef}
          style={[
          ...
          accessibilityLabel={accessibilityLabel || `Context menu${title ? ` for ${title}` : ''}`}
        >
          {/* Title */}
          {title && (
          ...
          <View style={styles.sectionsContainer}>
            {normalizedSections.map((section, sectionIndex) => renderSection(section, sectionIndex))}
          </View>
        </Animated.View>
      </Pressable>
    </Modal>
...
export const useContextMenu = () => {
  ...
  const [anchorPosition, setAnchorPosition] = React.useState<ContextMenuPosition | undefined>();
