    ...
    <Header
      title={
        <HeaderTitleWithSubtitle
          subtitle={
            serverInfo.isCustom ? serverInfo.hostname + (serverInfo.port ? `:${serverInfo.port}` : '') : undefined
          }
        />
      }
...
function HeaderRight() {
  ...
  // );
  return (
    <Pressable onPress={() => router.push('/settings')} hitSlop={15} style={styles.headerButton}>
      <Image
    ...
    <View style={styles.titleContainer}>
      ...
      {showConnectionStatus && (
        <View style={styles.statusContainer}>
          ...
          />
          <Text style={[styles.statusText, { color: connectionStatus.textColor }]}>{connectionStatus.text}</Text>
        </View>
      )}
    </View>
  );
