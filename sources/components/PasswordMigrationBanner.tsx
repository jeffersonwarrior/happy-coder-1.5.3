...
export function PasswordMigrationBanner() {
  ...
  const [hasPasswordProtection, setHasPasswordProtection] = React.useState<boolean | null>(null);
  const [bannerDismissed, setBannerDismissed] = useLocalSettingMutable('passwordMigrationBannerDismissed');
  ...
  if (hasPasswordProtection === null || hasPasswordProtection || bannerDismissed) {
    return null;
  }
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.input.background }]}>
      <View style={styles.content}>
        ...
        <View style={styles.textContainer}>
          <Text style={[styles.title, { color: theme.colors.text }]}>{t('passwordMigration.title')}</Text>
        ...
        {/* Dismiss button */}
        <Pressable style={styles.dismissButton} onPress={handleDismiss} hitSlop={10}>
...
export function CompactPasswordMigrationBanner() {
  ...
  return (
    <Pressable
      style={[styles.compactContainer, { backgroundColor: theme.colors.input.background }]}
      onPress={handlePress}
    >
      <View style={styles.compactContent}>
        <Ionicons name="shield-checkmark-outline" size={20} color="#34C759" />
        <Text style={[styles.compactText, { color: theme.colors.text }]}>{t('passwordMigration.compactTitle')}</Text>
