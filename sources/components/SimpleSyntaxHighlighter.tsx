...
const tokenizeCode = (code: string, language: string | null) => {
  ...
  const patterns = [
    ...
    {
      regex: /\b(0x[0-9a-fA-F]+|0b[01]+|0o[0-7]+|\d+\.?\d*(?:[eE][+-]?\d+)?)\b/g,
      type: 'number',
    },
    ...
    {
      regex: new RegExp(`\\b(${keywordSets.imports.join('|')})\\b`, 'g'),
      type: 'import',
    },
    {
      regex: new RegExp(`\\b(${keywordSets.controlFlow.join('|')})\\b`, 'g'),
      type: 'controlFlow',
    },
    {
      regex: new RegExp(`\\b(${keywordSets.keywords.join('|')})\\b`, 'g'),
      type: 'keyword',
    },
    {
      regex: new RegExp(`\\b(${keywordSets.types.join('|')})\\b`, 'g'),
      type: 'type',
    },
    {
      regex: new RegExp(`\\b(${keywordSets.modifiers.join('|')})\\b`, 'g'),
      type: 'modifier',
    },
    {
      regex: new RegExp(`\\b(${keywordSets.boolean.join('|')})\\b`, 'g'),
      type: 'boolean',
    },
...
export const SimpleSyntaxHighlighter: React.FC<SimpleSyntaxHighlighterProps> = ({ code, language, selectable }) => {
