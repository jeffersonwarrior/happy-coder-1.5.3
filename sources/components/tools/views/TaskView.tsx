      ...
      {visibleTools.map((item, index) => (
        <View key={`${item.tool.name}-${index}`} style={styles.toolItem}>
          ...
          <View style={styles.statusContainer}>
            ...
            {item.state === 'completed' && <Ionicons name="checkmark-circle" size={16} color={theme.colors.success} />}
            {item.state === 'error' && <Ionicons name="close-circle" size={16} color={theme.colors.textDestructive} />}
          </View>
        </View>
      ))}
      {remainingCount > 0 && (
        <View style={styles.moreToolsItem}>
          <Text style={styles.moreToolsText}>{t('tools.taskView.moreTools', { count: remainingCount })}</Text>
        </View>
      )}
    </View>
  );
