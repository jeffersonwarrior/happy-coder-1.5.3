...
export class MCPService {
   ...
   */
  async discoverModels(forceRefresh: boolean = false): Promise<MCPDiscoveryResult> {
    ...
    try {
      ...
      results.forEach((result, index) => {
        if (result.status === 'fulfilled') {
          ...
          errors.push(`Failed to discover ${providerNames[index]} models: ${result.reason}`);
        }
      });
      ...
      console.log(`✅ Discovered ${allModels.length} models from ${results.length} providers`);
    ...
    const models: MCPModel[] = [
      {
        ...
        limitations: {
          ...
          knownIssues: [],
          bestUseCases: ['Complex coding tasks', 'Code review', 'Architecture planning'],
      ...
      },
      {
        ...
        limitations: {
          ...
          knownIssues: [],
          bestUseCases: ['Quick code fixes', 'Simple explanations', 'Fast iterations'],
          notRecommendedFor: ['Complex system design', 'Large codebase analysis'],
        },
        contextWindow: 200000,
    ...
    const models: MCPModel[] = [
      {
        ...
        limitations: {
          ...
          bestUseCases: ['General programming', 'Multimodal tasks', 'Complex reasoning'],
      ...
      },
      {
        ...
        limitations: {
          ...
          notRecommendedFor: ['Complex system architecture', 'Advanced reasoning'],
        },
        contextWindow: 128000,
        maxOutputTokens: 16384,
        supportedLanguages: ['en', 'es', 'fr', 'de', 'it', 'pt', 'ru', 'ja', 'ko', 'zh'],
    ...
    const models: MCPModel[] = [
      {
        ...
        limitations: {
          ...
          bestUseCases: ['Long document analysis', 'Complex reasoning', 'Large codebase review'],
    ...
    if (__DEV__ || Platform.OS === 'web') {
      models.push({
        ...
        limitations: {
          ...
          unsupportedFeatures: ['Image analysis', 'Web search', 'Function calling'],
   ...
   */
  filterModelsByCapabilities(capabilities: Partial<ModelCapabilities>): MCPModel[] {
  ...
  getCacheInfo(): { size: number; entries: string[]; oldestEntry?: number } {
    ...
    const oldestEntry =
      entries.length > 0
        ? Math.min(...entries.map(key => this.cachedResults.get(key)?.discoveredAt || Date.now()))
        : undefined;
