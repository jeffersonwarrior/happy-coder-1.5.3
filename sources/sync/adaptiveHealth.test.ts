...
import { AdaptiveHealthConfig, AdaptiveHealthMonitor } from './adaptiveHealth';
...
describe('AdaptiveHealthMonitor', () => {
  ...
  describe('Oscillation Prevention', () => {
    ...
    it('should prevent rapid oscillation between intervals', () => {
      ...
      const intervalChanges = intervals.slice(1).map((interval, i) => Math.abs(interval - intervals[i]) / intervals[i]);
