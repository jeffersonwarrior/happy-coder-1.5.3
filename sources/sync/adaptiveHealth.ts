...
export class AdaptiveHealthMonitor {
  ...
  recordPingResult(result: PingResult): void {
    ...
    if (result.success) {
      ...
      console.log(
        `🔄 AdaptiveHealthMonitor: Ping success (${this.consecutiveSuccesses} consecutive), latency: ${result.latency}ms`
      );
    } else {
      this.consecutiveFailures++;
      this.consecutiveSuccesses = 0;
      console.log(
        `🔄 AdaptiveHealthMonitor: Ping failure (${this.consecutiveFailures} consecutive), error: ${result.error}`
      );
    }
  ...
  getStatus(): {
    ...
    return {
      ...
      successRate:
        this.pingHistory.length > 0 ? this.pingHistory.filter(r => r.success).length / this.pingHistory.length : 0,
    };
  }
  ...
  private adaptPingInterval(): void {
    ...
    console.log(
      `🔄 AdaptiveHealthMonitor: Adapting interval - stability: ${stability.toFixed(2)}, latency trend: ${latencyTrend.toFixed(2)}`
    );
    ...
    if (this.consecutiveFailures >= 2 || stability < 0.7) {
      ...
      this.currentInterval = Math.max(this.config.minPingInterval, this.currentInterval * 0.8);
      ...
      this.currentInterval = Math.min(this.config.maxPingInterval, this.currentInterval * 1.1);
    ...
    if (changePercent > 0.1) {
      console.log(
        `🔄 AdaptiveHealthMonitor: Interval changed from ${oldInterval}ms to ${this.currentInterval}ms (${(changePercent * 100).toFixed(1)}% change)`
      );
  ...
  private calculateStability(): number {
    ...
    const latencyStability = avgLatency > 0 ? Math.max(0, 1 - Math.sqrt(variance) / avgLatency) : 1.0;
  ...
  getAnalytics(): {
    ...
    const avgLatency =
      successfulPings.length > 0 ? successfulPings.reduce((sum, r) => sum + r.latency!, 0) / successfulPings.length : 0;
