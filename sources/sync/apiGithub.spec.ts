...
describe('apiGithub', () => {
  ...
  describe('disconnectGitHub', () => {
    ...
    it('should throw error when GitHub account is not connected', async () => {
      ...
      await expect(disconnectGitHub(mockCredentials)).rejects.toThrow('GitHub account not connected');
    });
    ...
    it('should throw error when server returns non-success response', async () => {
      ...
      await expect(disconnectGitHub(mockCredentials)).rejects.toThrow('Failed to disconnect GitHub account');
    });
    ...
    it('should throw generic error for other HTTP errors', async () => {
      ...
      await expect(disconnectGitHub(mockCredentials)).rejects.toThrow('Failed to disconnect GitHub: 500');
    });
  });
});
