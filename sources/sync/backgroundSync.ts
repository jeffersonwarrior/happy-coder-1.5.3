...
import { EXPO_MODULES_AVAILABILITY, getBackgroundFetch, getTaskManager } from './expoModuleMocks';
...
export class BackgroundSyncManager {
  ...
  private criticalSyncQueue: Array<{
    operation: string;
    data: any;
    timestamp: number;
  }> = [];
  ...
  constructor(private config: BackgroundSyncConfig = DEFAULT_BACKGROUND_CONFIG) {
    ...
    log.log(
      `📱 Background sync initialized. TaskManager: ${EXPO_MODULES_AVAILABILITY.taskManager ? '✅' : '❌'}, BackgroundFetch: ${EXPO_MODULES_AVAILABILITY.backgroundFetch ? '✅' : '❌'}`
    );
  }
  ...
  private async startIOSBackgroundTask() {
    ...
    this.connectionHealthInterval = setInterval(
      ...
      Math.max(this.config.syncInterval, 30000)
    ); // Minimum 30s interval for iOS
  }
  ...
  private async syncCriticalData() {
    try {
      ...
      const recentOperations = this.criticalSyncQueue.filter(op => now - op.timestamp < 300000);
  ...
  private async cleanupStaleConnections() {
    try {
      ...
      this.criticalSyncQueue = this.criticalSyncQueue.filter(op => now - op.timestamp < 600000);
