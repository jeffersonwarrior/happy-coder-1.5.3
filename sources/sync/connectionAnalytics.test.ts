...
import { ConnectionAnalytics, ConnectionEvent } from './connectionAnalytics';
...
describe('Connection Analytics', () => {
  ...
  describe('Failure Pattern Detection', () => {
    test('should identify failure patterns', () => {
      ...
      [...timeouts, ...networkErrors].forEach(event => analytics.recordConnectionEvent(event));
  ...
  describe('Performance Report Generation', () => {
    test('should provide actionable recommendations', () => {
      ...
      expect(
        report.recommendations.some(r => r.includes('cellular') || r.includes('timeout') || r.includes('aggressive'))
      ).toBe(true);
    });
