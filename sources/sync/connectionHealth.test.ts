...
import { ConnectionHealthConfig, ConnectionHealthMonitor, HEARTBEAT_PROFILES } from './connectionHealth';
...
describe('ConnectionHealthMonitor - Heartbeat Profiles', () => {
  ...
  describe('Profile Management', () => {
    ...
    it('should handle unknown profile names gracefully', () => {
      ...
      const currentProfile = monitor.getCurrentProfile();
      expect(currentProfile.name).toBe('standard');
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("Unknown profile 'unknown'"));
    ...
    it('should return all available profiles', () => {
      ...
      expect(Object.keys(availableProfiles)).toEqual(['standard', 'aggressive', 'corporate', 'battery_saver']);
  ...
  describe('Heartbeat Reconfiguration', () => {
    ...
    it('should reconfigure heartbeat when profile changes', () => {
      ...
      expect(setIntervalSpy).toHaveBeenCalledWith(expect.any(Function), HEARTBEAT_PROFILES.aggressive.interval);
    });
    ...
    it('should use current profile interval when starting monitoring', () => {
      ...
      expect(setIntervalSpy).toHaveBeenCalledWith(expect.any(Function), HEARTBEAT_PROFILES.corporate.interval);
    });
  ...
  describe('Auto-Detection Logic', () => {
    ...
    it('should detect corporate profile for high failure rate', () => {
      ...
      monitor_any.failureHistory = Array(10)
        .fill(null)
        .map((_, i) => ({
    ...
    it('should detect aggressive profile for moderate failure rate', () => {
      const monitor_any = monitor as any;
      monitor_any.failureHistory = Array(3)
        .fill(null)
        .map((_, i) => ({
          timestamp: Date.now() - i * 1000,
          type: 'network_error',
    ...
    it('should detect standard profile for normal conditions', () => {
      const monitor_any = monitor as any;
      monitor_any.failureHistory = Array(1)
        .fill(null)
        .map((_, i) => ({
    ...
    it('should apply auto-detected profile when different from current', () => {
      ...
      const monitor_any = monitor as any;
      monitor_any.failureHistory = Array(12)
        .fill(null)
        .map((_, i) => ({
          timestamp: Date.now() - i * 1000,
          type: 'timeout',
  ...
  describe('Performance Requirements', () => {
    ...
    it('should clean up old failure history', () => {
      ...
      monitor_any.failureHistory = monitor_any.failureHistory.filter((f: any) => f.timestamp > oneHourAgo);
