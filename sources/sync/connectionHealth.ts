...
import { ConnectionState, connectionStateMachine, type StateTransitionEvent } from './connectionStateMachine';
...
export class ConnectionHealthMonitor {
   ...
   */
  getCurrentProfile(): {
    name: keyof typeof HEARTBEAT_PROFILES;
  ...
  private async performHealthCheck(): Promise<void> {
    ...
    try {
      ...
      console.warn(
        `🏥 ConnectionHealthMonitor: Ping failed (${this.status.consecutiveFailures}/${currentProfileData.maxConsecutiveFailures})`,
        error
      );
  ...
  private updateTimingMetrics(): void {
    ...
    if (this.lastDisconnectTime) {
      this.status.downtime =
        this.totalDowntime + (this.connectionStartTime ? this.connectionStartTime - this.lastDisconnectTime : 0);
    }
  }
