...
export class ConnectionTimeoutHandler {
   ...
   */
  async requestWithTimeout<T = any>(url: string, options: RequestOptions = {}): Promise<T> {
    const {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        const delay = this.calculateDelay(attempt);
        console.warn(
          `🌐 ConnectionTimeoutHandler: Request failed (attempt ${attempt}), retrying in ${delay}ms:`,
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.name === 'AbortError' || error.message.includes('aborted')) {
          const timeoutError: TimeoutError = Object.assign(new Error(`Request timeout after ${timeout}ms`), {
 ...
 */
export function requestWithTimeout<T = any>(url: string, options: RequestOptions = {}): Promise<T> {
