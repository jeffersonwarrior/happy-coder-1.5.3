...
export async function forceStopDaemon(machineId: string): Promise<DaemonControlResult> {
  try {
    ...
    const successfulKills = killResults.filter(result => result.status === 'fulfilled' && result.value.success).length;
...
export async function removeSessionLocally(machineId: string): Promise<DaemonControlResult> {
  try {
    const sessions = storage.getState().sessions;
    const machineSessions = Object.values(sessions).filter(session => session.metadata?.machineId === machineId);
...
export function hasActiveSessions(machineId: string): boolean {
  const sessions = storage.getState().sessions;
  return Object.values(sessions).some(session => session.metadata?.machineId === machineId && session.active);
}
...
export function getActiveSessionCount(machineId: string): number {
  const sessions = storage.getState().sessions;
  return Object.values(sessions).filter(session => session.metadata?.machineId === machineId && session.active).length;
}
