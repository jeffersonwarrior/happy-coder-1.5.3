...
export class NetworkDetection {
  ...
  private async createNetworkProfile(netInfo: NetInfoState): Promise<NetworkProfile> {
    ...
    const quality = netInfo.isConnected && netInfo.isInternetReachable ? await this.assessNetworkQuality() : 'unknown';
  ...
  private async assessNetworkQuality(): Promise<NetworkProfile['quality']> {
    const testPromises = this.config.qualityTestUrls.map(url => this.performLatencyTest(url));

    try {
      const results = await Promise.allSettled(testPromises);
      const successfulResults = results
        .filter(
          (result): result is PromiseFulfilledResult<LatencyTestResult> =>
      ...
      const avgLatency =
        successfulResults.reduce((sum, result) => sum + (result.latency || 0), 0) / successfulResults.length;
  ...
  private calculateStability(): number {
    ...
    const successfulLatencies = recentResults.filter(r => r.success && r.latency !== null).map(r => r.latency!);
    ...
    const variance =
      successfulLatencies.reduce((acc, lat) => acc + Math.pow(lat - avgLatency, 2), 0) / successfulLatencies.length;
   ...
   */
  private adjustStrategyForStability(strategy: ConnectionStrategy, profile: NetworkProfile): ConnectionStrategy {
   ...
   */
  private adjustStrategyForCellularGeneration(strategy: ConnectionStrategy, generation: string): ConnectionStrategy {
    const adjusted = JSON.parse(JSON.stringify(strategy)) as ConnectionStrategy;
  ...
  getStatistics(): NetworkDetectionStatistics {
    return {
      ...
      averageLatency:
        this.latencyHistory.length > 0
          ? this.latencyHistory.filter(r => r.success && r.latency !== null).reduce((sum, r) => sum + r.latency!, 0) /
