...
describe('NetworkDetection Integration', () => {
  ...
  const mockNetInfoAddEventListener = mockNetInfo.addEventListener as MockedFunction<
    typeof mockNetInfo.addEventListener
  >;
  ...
  describe('Network Change Detection and Strategy Adaptation', () => {
    it('should detect network changes and adapt strategies automatically', async () => {
      ...
      mockFetch.mockImplementation(
        () =>
          new Promise(
            resolve => setTimeout(() => resolve(new Response('', { status: 200 })), 50) // 50ms = excellent
          )
      );
      ...
      mockFetch.mockImplementation(
        () =>
          new Promise(
            resolve => setTimeout(() => resolve(new Response('', { status: 200 })), 600) // 600ms = poor
          )
      );
      ...
      expect(wifiStrategy!.strategy.timeouts.connection).toBeLessThan(cellularStrategy!.strategy.timeouts.connection);
...
describe('Real-world Network Scenarios', () => {
  ...
  it('should handle corporate firewall scenario', async () => {
    ...
    mockFetch.mockImplementation(() => {
      ...
      return new Promise(
        resolve => setTimeout(() => resolve(new Response('', { status: 200 })), 800) // Slow corporate network
      );
    });
  ...
  it('should optimize for 5G networks', async () => {
    ...
    mockFetch.mockImplementation(
      () =>
        new Promise(
          resolve => setTimeout(() => resolve(new Response('', { status: 200 })), 25) // Very fast
        )
    );
