...
export class StaleConnectionCleaner {
  ...
  private async performCleanup(): Promise<CleanupResult> {
    ...
    console.log(`🧹 StaleConnectionCleaner: Found ${staleSessions.length} stale sessions`);
  ...
  private async cleanupSession(session: Session): Promise<boolean> {
    ...
    if (retryCount >= this.config.maxRetries) {
      console.log(`🧹 StaleConnectionCleaner: Session ${sessionId} has exceeded max retries (${retryCount})`);
      return false;
    }
    ...
    try {
      ...
      if (isAlive) {
        console.log(`🧹 StaleConnectionCleaner: Session ${sessionId} is actually alive, skipping cleanup`);
  ...
  private async markSessionInactive(sessionId: string): Promise<void> {
    ...
    if (session) {
      ...
      try {
        ...
        console.warn('🧹 StaleConnectionCleaner: Failed to sync session state to server:', error);
