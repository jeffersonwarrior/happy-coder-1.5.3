...
import { applyLocalSettings, LocalSettings } from './localSettings';
import {
  loadLocalSettings,
  loadProfile,
  loadPurchases,
...
import { customerInfoToPurchases, Purchases } from './purchases';
import { createReducer, ReducerState, reducer } from './reducer/reducer';
import type { CustomerInfo } from './revenueCat/types';
import { applySettings, Settings } from './settings';
import { GitStatus, Machine, Session } from './storageTypes';
      ...
      set(state => {
        ...
        sessions.forEach(session => {
          ...
          if (
            ...
            if (currentRealtimeSessionId === session.id && voiceSession) {
              ...
              for (const [requestId, request] of Object.entries(newRequests)) {
                if (!oldRequests[requestId]) {
                  ...
                  voiceSession.sendTextMessage(`Claude is requesting permission to use the ${toolName} tool`);
                }
              }
            }
            ...
            const messagesArray = Object.values(mergedMessagesMap).sort((a, b) => b.createdAt - a.createdAt);
