...
export class ErrorHandler {
   ...
   */
  static async wrapAsync<T>(operation: () => Promise<T>, context: ErrorContext): Promise<T> {
    try {
      return await operation();
    } catch (error) {
      const wrappedError = this.wrapError(error, context);
      log.error(`Operation failed: ${context.operation} - Source: ${context.source} - Error: ${wrappedError}`);
      throw wrappedError;
    }
  }
   ...
   */
  static async withTimeout<T>(promise: Promise<T>, timeoutMs: number, context: ErrorContext): Promise<T> {
    const timeoutPromise = new Promise<never>((_, reject) => {
      setTimeout(() => {
        reject(new TimeoutError(`Operation timed out after ${timeoutMs}ms`, timeoutMs, context));
      }, timeoutMs);
    });
   ...
   */
  static handleError(error: unknown, context: ErrorContext, recovery?: () => void): ConnectionError {
    const wrappedError = this.wrapError(error, context);

    log.error(`Error in ${context.operation} - Source: ${context.source} - Error: ${wrappedError}`);
  ...
  static async withRetry<T>(
    ...
    const { maxRetries, baseDelayMs, backoffMultiplier = 2, jitter = true, context } = options;
    let lastError: Error;
  ...
  private static isTimeoutError(error: Error): boolean {
    ...
    return message.includes('timeout') || message.includes('timed out') || error.name === 'TimeoutError';
  }
...
export function withErrorHandling(context: Partial<ErrorContext>) {
  return function (target: any, propertyName: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      return ErrorHandler.wrapAsync(() => method.apply(this, args), fullContext);
    };
  };
}
