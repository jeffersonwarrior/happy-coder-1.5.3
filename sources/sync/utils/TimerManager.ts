...
export class TimerManager {
   ...
   */
  setTimeout(callback: () => void, delay: number, id: string, options?: TimerOptions): any {
    ...
    const wrappedCallback = () => {
      try {
        ...
        log.error(`Timer callback error for ${id} - Source: ${this.source} - Error: ${err}`);
    ...
    if (options?.name) {
      log.log(`DEBUG: Set timeout '${options.name}' (${id}) for ${delay}ms - Source: ${this.source}`);
    }
   ...
   */
  setInterval(callback: () => void, interval: number, id: string, options?: TimerOptions): any {
    ...
    const wrappedCallback = () => {
      try {
        ...
        const err = error instanceof Error ? error : new Error(String(error));
        log.error(`Interval callback error for ${id} - Source: ${this.source} - Error: ${err}`);
    ...
    if (options?.name) {
      log.log(`DEBUG: Set interval '${options.name}' (${id}) for ${interval}ms - Source: ${this.source}`);
    }
  ...
  clearTimeout(id: string): void {
    ...
    if (timer) {
      clearTimeout(timer);
      this.timers.delete(id);
      log.log(`DEBUG: Cleared timeout ${id} - Source: ${this.source}`);
    }
  }
  ...
  clearInterval(id: string): void {
    ...
    if (timer) {
      ...
      log.log(`DEBUG: Cleared interval ${id} - Source: ${this.source}`);
    }
  }
  ...
  clearAll(): void {
    ...
    log.log(`DEBUG: Cleared all timers - Source: ${this.source}`);
  }
   ...
   */
  restartInterval(callback: () => void, interval: number, id: string, options?: TimerOptions): any {
    this.clearInterval(id);
   ...
   */
  rescheduleTimeout(callback: () => void, delay: number, id: string, options?: TimerOptions): any {
    this.clearTimeout(id);
...
export class TimerUtils {
  ...
  static async withBackoff<T>(
    ...
    const { maxRetries, baseDelay, backoffMultiplier = 2, jitter = true, onRetry } = options;
