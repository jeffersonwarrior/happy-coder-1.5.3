...
export const pl: TranslationStructure = {
  ...
  errors: {
    ...
    errorWithCode: ({ message, code }: { message: string; code: number | string }) => `${message} (Błąd ${code})`,
    disconnectServiceFailed: ({ service }: { service: string }) => `Nie udało się rozłączyć ${service}`,
  ...
  sessions: {
    ...
    deleteSessionMessage: ({ sessionName }: { sessionName: string }) =>
      `Czy na pewno chcesz usunąć "${sessionName}"? Tej operacji nie można cofnąć.`,
  ...
  password: {
    ...
    incorrectPasswordMessage: ({ attempts }: { attempts: number }) =>
      `Nieprawidłowe hasło. ${plural({ count: attempts, one: 'Pozostała 1 próba', few: `Pozostały ${attempts} próby`, many: `Pozostało ${attempts} prób` })}.`,
    tooManyAttempts: 'Zbyt wiele prób',
    tooManyAttemptsMessage:
      'Zbyt wiele nieudanych prób wprowadzenia hasła. Spróbuj uwierzytelniania biometrycznego lub zresetuj aplikację.',
    attemptsRemaining: ({ attempts }: { attempts: number }) =>
      plural({
        count: attempts,
        one: 'Pozostała 1 próba',
        few: `Pozostały ${attempts} próby`,
        many: `Pozostało ${attempts} prób`,
      }),
    ...
    forgotPasswordMessage:
      'Zapomnienie hasła spowoduje zresetowanie aplikacji i konieczność przywrócenia konta za pomocą klucza tajnego.',
    ...
    resetPasswordBiometric:
      'Twoja tożsamość została zweryfikowana. Spowoduje to usunięcie ochrony hasłem, zachowując bezpieczeństwo danych sesji.',
    resetPassword: 'Zresetuj hasło',
    resetAppTitle: 'Resetowanie aplikacji',
    resetAppWarning:
      'To usunie WSZYSTKIE twoje dane, w tym sesje, ustawienia i informacje o koncie. Tej operacji nie można cofnąć.',
  ...
  passwordMigration: {
    ...
    description:
      'Dodaj ochronę hasłem, aby zabezpieczyć swoje sesje programowania. Twoje dane pozostaną prywatne z opcją odblokowania biometrycznego.',
